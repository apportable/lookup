...
si_mod_t *si_module_static_system(void);
...
#endif
		{ "system", si_module_static_system, NULL },
		{ NULL, NULL },
	};
...
#ifdef APPORTABLE // removing read rights dont deallocate the port yet....
#warning known issue with mach_port_mod_refs - https://code.google.com/p/apportable/issues/detail?id=545
	mach_port_deallocate(mach_task_self(), r->port);
#else
