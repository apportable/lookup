...
typedef struct
{
	...
	uint32_t netgroup_validation_a;
	uint32_t netgroup_validation_b;
} file_si_private_t;
...
static void
_fsi_get_validation(si_mod_t *si, int vtype, const char *path, FILE *f, uint32_t *a, uint32_t *b)
{
	struct stat sb;
	...
	else
	{
		...
		if (f != NULL)
		{
			...
			{
#ifdef ANDROID
				if (a != NULL) *a = sb.st_mtime;
				if (b != NULL) *b = sb.st_mtime_nsec;
#else
...
static int
_fsi_validate(si_mod_t *si, int cat, uint32_t va, uint32_t vb)
{
	struct stat sb;
	const char *path;
...
#ifdef ANDROID
		if (va != sb.st_mtime) return 0;
		if (vb != sb.st_mtime_nsec) return 0;
#else
...
static si_item_t *
_fsi_parse_user(si_mod_t *si, const char *name, uid_t uid, int which, char *data, int format, uint32_t va, uint32_t vb)
{
	char **tokens;
	int ntokens, match;
	...
	if (format == 0)
	{
		...
		item = (si_item_t *)LI_ils_create("L4444ss44LssssL", (unsigned long)si, CATEGORY_USER, 1, va, vb, tokens[0], tokens[1], xuid, atoi(tokens[3]), change, tokens[4], tokens[7], tokens[8], tokens[9], expire);
	}
	else
	{
		...
		item = (si_item_t *)LI_ils_create("L4444ss44LssssL", (unsigned long)si, CATEGORY_USER, 1, va, vb, tokens[0], tokens[1], xuid, atoi(tokens[3]), 0, "", tokens[4], tokens[5], tokens[6], 0);
	}
...
static si_item_t *
_fsi_parse_group(si_mod_t *si, const char *name, gid_t gid, int which, char *data, uint32_t va, uint32_t vb)
{
	char **tokens, **members;
	int ntokens, match;
	si_item_t *item;
	...
	item = (si_item_t *)LI_ils_create("L4444ss4*", (unsigned long)si, CATEGORY_GROUP, 1, va, vb, tokens[0], tokens[1], xgid, members);
...
_fsi_get_grouplist(si_mod_t *si, const char *user)
{
	...
	if (gidcount != 0) {
		item = (si_item_t *)LI_ils_create("L4444s4@", (unsigned long)si, CATEGORY_GROUPLIST, 1, va, vb, user, gidcount, 
...
static si_item_t *
_fsi_parse_alias(si_mod_t *si, const char *name, int which, char *data, uint32_t va, uint32_t vb)
{
	char **tokens, **members;
	int ntokens, match;
	si_item_t *item;
	...
	item = (si_item_t *)LI_ils_create("L4444s4*4", (unsigned long)si, CATEGORY_ALIAS, 1, va, vb, tokens[0], ntokens, members, 1);
...
static si_item_t *
_fsi_parse_ether(si_mod_t *si, const char *name, int which, char *data, uint32_t va, uint32_t vb)
{
	char **tokens;
	char *cmac;
	int ntokens, match;
	...
	item = (si_item_t *)LI_ils_create("L4444ss", (unsigned long)si, CATEGORY_MAC, 1, va, vb, tokens[1], cmac);
...
static si_item_t *
_fsi_parse_host(si_mod_t *si, const char *name, const void *addr, int af, int which, char *data, uint32_t va, uint32_t vb)
{
	...
	{
		item = (si_item_t *)LI_ils_create("L4444s*44a", (unsigned long)si, CATEGORY_HOST_IPV4, 1, va, vb, tokens[1], h_aliases, h_addrtype, h_length, h_addr_list);
	}
	else
	{
		item = (si_item_t *)LI_ils_create("L4444s*44c", (unsigned long)si, CATEGORY_HOST_IPV6, 1, va, vb, tokens[1], h_aliases, h_addrtype, h_length, h_addr_list);
	}
...
static si_item_t *
_fsi_parse_service(si_mod_t *si, const char *name, const char *proto, int port, int which, char *data, uint32_t va, uint32_t vb)
{
	...
	*xproto++ = '\0';
	...
	item = (si_item_t *)LI_ils_create("L4444s*4s", (unsigned long)si, CATEGORY_SERVICE, 1, va, vb, tokens[0], s_aliases, xport, xproto);
...
static si_item_t *
_fsi_parse_name_num_aliases(si_mod_t *si, const char *name, int num, int which, char *data, uint32_t va, uint32_t vb, int cat)
{
	char **tokens, **aliases;
	...
	switch (cat) {
		case CATEGORY_NETWORK:
			// struct netent
			item = (si_item_t *)LI_ils_create("L4444s*44", (unsigned long)si, cat, 1, va, vb, tokens[0], aliases, AF_INET, xnum);
			break;
		case CATEGORY_PROTOCOL:
		case CATEGORY_RPC:
			// struct protoent
			// struct rpcent
			item = (si_item_t *)LI_ils_create("L4444s*4", (unsigned long)si, cat, 1, va, vb, tokens[0], aliases, xnum);
			break;
		default:
			abort();
	}
...
static si_item_t *
_fsi_parse_fs(si_mod_t *si, const char *name, int which, char *data, uint32_t va, uint32_t vb)
{
	...
	item = (si_item_t *)LI_ils_create("L4444sssss44", (unsigned long)si, CATEGORY_FS, 1, va, vb, tokens[0], tokens[1], tokens[2], tokens[3], (fstype == NULL) ? "rw" : fstype, freq, passno);
...
_fsi_fs_root(si_mod_t *si)
{
	dispatch_once(&rootfs_once, ^{
		...
		if (stat(root_path, &rootstat) < 0) return;
 		...
 		const char *fstypename = "default";
 #ifndef ANDROID		
		// Check to make sure we're not looking at a synthetic root:
 		fstypename = rootfsinfo.f_fstypename;

		if (string_equal(fstypename, "synthfs")) {
			root_path = "/root";
        		if (stat(root_path, &rootstat) < 0) return;
			if (statfs(root_path, &rootfsinfo) < 0) return;
		}
		...
		rootfs = (si_item_t *)LI_ils_create("L4444sssss44", (unsigned long)si, CATEGORY_FS, 1, 0, 0, root_spec, root_path, fstypename, FSTAB_RW, FSTAB_RW, 0, 1);
	});
...
file_netgroup_byname(si_mod_t *si, const char *name)
{
	...
	if (n != NULL)
	{
		...
		while (m != NULL)
		{
			item = (si_item_t *)LI_ils_create("L4444sss", (unsigned long)si, CATEGORY_NETGROUP, 1, va, vb, m->host, m->user, m->domain);
