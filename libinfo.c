...
extern int REALFN(initgroups)(u_int gidsetsize, gid_t *gidset, int gmuid);
...
int 
WRAPFN(getgrouplist)(const char *name, gid_t basegid, gid_t *groups, int *ngroups)
{
#if DS_AVAILABLE
...
int
WRAPFN(initgroups)(const char *name, gid_t basegid)
{
	int status;
	uint32_t ngroups;
	...
	status = REALFN(initgroups)(ngroups, groups, uid);
...
struct hostent *
WRAPFN(gethostbyname)(const char *name)
{
	si_item_t *item;
	uint32_t status;
	struct in_addr addr4;
...
struct hostent *
WRAPFN(gethostbyname2)(const char *name, int af)
{
	si_item_t *item;
	uint32_t status;
	struct in_addr addr4;
...
struct hostent *
WRAPFN(gethostbyaddr)(const void *addr, int len, int type)
{
	si_item_t *item;
	uint32_t status;
...
struct servent *
WRAPFN(getservbyname)(const char *name, const char *proto)
{
	si_item_t *item;
...
int
WRAPFN(getaddrinfo)(const char *nodename, const char *servname, const struct addrinfo *hints, struct addrinfo **res)
{
...
int
WRAPFN(getnameinfo)(const struct sockaddr *sa, socklen_t salen, char *node, size_t nodelen, char *service, size_t servicelen, int flags)
{
